...
//빠짐없이 점수가 부여됐을 경우
	...
	//리뷰 토탈 스코어 변화
	$getoriginScore = "SELECT SUM(BeerScore) FROM Beer_Review WHERE Review_Beer_ID=$Review_Beer_ID";
	$originTotalScore = mysqli_fetch_array(mysqli_query($db,$getoriginScore));
	$reviewer = "SELECT COUNT(Review_ID) FROM Beer_Review WHERE Review_Beer_ID=$Review_Beer_ID";
	$NoReviewer = mysqli_fetch_array(mysqli_query($db,$reviewer));
	$newTotalScore = ROUND((($stars_TotalScore + $originTotalScore[0])/($NoReviewer[0]+1)),2);
	$insertquery = "UPDATE Beer_Score SET Review_TotalScore='$newTotalScore' WHERE Beer_ID='$Review_Beer_ID'";
